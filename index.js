...
// Helper function to extract private keys from .env
function getPrivateKeysFromEnv() {
    // Check for multiple private keys first (WALLET_PRIVATE_KEY_1, WALLET_PRIVATE_KEY_2, etc.)
    const multiKeyPattern = /^WALLET_PRIVATE_KEY_\d+$/;
    const multiKeys = Object.keys(process.env)
        .filter(key => multiKeyPattern.test(key))
        .map(key => process.env[key])
        .filter(key => key && key !== 'your_private_key_here');
    
    // If we found multiple keys with the pattern, return those
    if (multiKeys.length > 0) {
        return multiKeys;
    }
    
    // Otherwise, check for the single key format
    const singleKey = process.env.WALLET_PRIVATE_KEY;
    if (singleKey && singleKey !== 'your_private_key_here') {
        return [singleKey];
    }
    
    // If no valid keys found, return empty array
    return [];
}
...
// Add this function to get user input with wallet selection
async function getUserInput() {
    ...
    try {
        ...
        if (privateKeys.length === 0) {
            throw new Error('No valid private keys found in .env file. Please add them as WALLET_PRIVATE_KEY or WALLET_PRIVATE_KEY_1, WALLET_PRIVATE_KEY_2, etc.');
        }
        ...
        for (let i = 0; i < privateKeys.length; i++) {
            ...
            const tempProvider = new ethers.JsonRpcProvider(CONFIG.RPC_URL);
            const tempWallet = new ethers.Wallet(privateKeys[i], tempProvider);
            console.log(`${i + 1}. ${tempWallet.address}`);
        }
        
        // Wallet selection
        let selectedWalletIndex;
        if (privateKeys.length === 1) {
            console.log('\nOnly one wallet available. Using it automatically.');
            selectedWalletIndex = 0;
        } else {
            const walletChoice = await question(`\nSelect wallet (1-${privateKeys.length}): `);
            selectedWalletIndex = parseInt(walletChoice) - 1;
            
            if (isNaN(selectedWalletIndex) || selectedWalletIndex < 0 || selectedWalletIndex >= privateKeys.length) {
                throw new Error(`Please enter a valid wallet number between 1 and ${privateKeys.length}`);
            }
        }
        ...
        return {
            privateKey: privateKeys[selectedWalletIndex],
...
// Add this function to run with multi-wallet support
async function runMultiWalletBot() {
    try {
        // Get user input including wallet selection
        const { privateKey, txCount, delayInSeconds } = await getUserInput();

        console.log('\n🚀 Initializing Swap Bot...');
        const bot = new ZeroGSwapBot(privateKey);
        const initialized = await bot.initialize();
        
        if (!initialized) {
            throw new Error('Failed to initialize bot with selected wallet');
        }
